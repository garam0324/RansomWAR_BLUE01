...
// 여기서 P(x)는 특정 바이트 값(0~255)이 등장할 확률입니다.
static double calculate_entropy(const char *buf, size_t size) {
    if (size == 0) return 0.0; // 크기가 0인 버퍼는 entropy가 0 (예외 처리)
...
static unsigned long read_and_increment_backup_count(const char *backup_dir_path) {
    ...
    if (fp) {
        if (fscanf(fp, "%lu", &count) != 1) { // 기존 ㅊount 값 읽기
            count = 0;
        }
        fclose(fp);
    }
...
static int create_snapshot(const char *path, const char *relpath) {
    ...
    // 경로 구분자를 ('/') 대신 ('_')를 사용하여 경로를 하나의 파일명 처럼 관리
...
static int myfs_write(const char *path, const char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
    ...
    if (state) {
        ...
        // 엔트로피 검사
        // 정상적인 txt나 code는 패턴이 존재하여 낮은 entropy를 갖는다.
        // 랜섬웨어에 의해 암호화된 데이터는 높은 entropy를 갖기때문에 해당 코드에서는 7.5이상의 entropy를 갖는 행위에 대하여 차단한다
        ...
        if (entropy > HIGH_ENTROPY_HARD_BLOCK) {
			...
			if (state) {
				pthread_mutex_lock(&state_mutex);
				state->blocked = 1;
				state->blocked_until = now_s + FILE_BLOCK_COOLDOWN_SEC; // 10초간 차단
				pthread_mutex_unlock(&state_mutex);
			}
        ...
        while (i < state->ts_count) {
            if (difftime(now_s, state->write_timestamps[i]) > WRITE_FREQUENCY_WINDOW) {
				// 윈도우 밖의 오래된 타임스탬프 제거
                state->write_timestamps[i] = state->write_timestamps[state->ts_count - 1]; // 제거로 인하여 배열의 순서를 당긴다.
                state->ts_count--;
            } else {
                i++;
            }
