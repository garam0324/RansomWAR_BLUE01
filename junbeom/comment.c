...
static int myfs_create(const char *path, mode_t mode, struct fuse_file_info *fi) {
    ...
    if (!is_whitelisted_and_has_ext(rel)) {    // 화이트리스트에 없는 확장자일 경우
        log_line("CREATE", rel, "BLOCKED",     // 로그 기록
                 "File extension not in whitelist policy (e.g., .exe or no extension)", NULL);
        return -EPERM;                         // 권한 없음 반환 
    }

    //  랜섬노트 이름 차단
    if (is_ransom_note(rel)) {                 // 파일명이 readme, decrypt 등 포함 시
        log_line("CREATE", rel, "BLOCKED",
                 "Ransom note name pattern detected", NULL); // 로그기록
        return -EPERM; // 권한 없음 반환
    }
    ...
    if (fd == -1) {                            // 생성 실패 시
        log_line("CREATE", rel, "DENY", "os-error", "errno=%d", errno); // 로그기록
        return -errno;
    }
    fi->fh = fd;                               // 파일 핸들 저장
...
static int myfs_rename(const char *from, const char *to, unsigned int flags) {
    ...
    // 새 이름(to)이 화이트리스트 확장자에 없는 경우 차단
    ...
    // 원본 파일의 실제 내용 확인 (일반파일 일 때만 검사)
    if (fstatat(base_fd, relfrom, &st, AT_SYMLINK_NOFOLLOW) == 0 && S_ISREG(st.st_mode)) {
        n_read = read_file_magic_at_base(relfrom, h, sizeof(h)); // 앞 16바이트 읽기

        if (n_read > 0) {
            // 매직넘버와 확장자 불일치 시 차단
            if (!magic_ok_for_ext(ext, h, n_read)) { // 확장자와 매직넘버의 일치 여부 판단
                pthread_mutex_unlock(&rename_lock);
                log_line("RENAME", relto, "BLOCKED",
                         "Deep Magic number mismatch (16-byte signature failed for new extension)", NULL);
                return -EPERM;
            }
        }
    }

    // 모든 정책 통과 → 실제 rename 수행
    ...
    // rename 허용 로그 기록
    log_line("RENAME", relto, "ALLOW", "Rename successful (per file count updated)",
             "old_path=%s", relfrom);
