...
static int unlink_rate_limit_exceeded(int *out_count_in_window) {
    ...
    if (rl_window_start == 0 || difftime(now, rl_window_start) >= UNLINK_WINDOW_SEC) { // 현재 시간(now)이 기존 윈도우 시작 시각으로부터 UNLINK_WINDOW_SEC만큼 지났다면
        rl_window_start = now; // 새 윈도우 열기
        rl_unlink_count = 0; // 카운트 초기화 
    }
    rl_unlink_count++; // 카운트 1 증가
    if (out_count_in_window) *out_count_in_window = rl_unlink_count;
    return (rl_unlink_count > MAX_UNLINK_PER_WINDOW); // 초과 여부 반환
}
...
static int myfs_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
    ...
    // 실제로 채워줄 길이 계산
    // 요청 size와 파일 끝까지 남은 길이(real_size - offset) 중 큰 값
    size_t to_read = size;
